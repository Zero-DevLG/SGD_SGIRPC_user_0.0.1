...
$consulta = $pdo->prepare("SELECT de.estatus,opt.bis,de.id_documento,opt.num,de.folio,de.remitente,di.asunto,de.cargo_r,de.n_oficio,de.fecha_oficio,di.fecha_limite FROM documentos_externos as de INNER JOIN documento_instruccion as di ON de.id_documento = di.id_documento INNER JOIN op_control as opt ON opt.id_documento = de.id_documento WHERE de.estatus =2 ");
$consulta->execute();
